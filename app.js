// app.js — App de Finanzas con Firebase (Auth + Firestore) + depuración expuesta
// ACTUALIZADO: balance/reportes restan también Gastos Personales, Pagos y Ordinarios (sin duplicar)
...
enableIndexedDbPersistence(db).catch(()=>{ /* ok si no hay persistencia */ });
// const appCheck = initializeAppCheck(fbApp, { provider: new ReCaptchaV3Provider("TU_SITE_KEY"), isTokenAutoRefreshEnabled: true });
...
function applyTheme(){
  ...
  // Fallback logo (si falta assets/logo.png)
...
function renderIncomes(){
  ...
  state.incomesDaily.filter(r=>inRange(r.date, from, to)).forEach(r=>{
    ...
    tr.innerHTML = `
      ...
      <td>${fmt(r.amount)}</td><td class="row-actions"><button class="btn-outline" data-del="${r.id}">Eliminar</button></td>`;
...
function wirePayments(){
  $('#paymentForm')?.addEventListener('submit',(ev)=>{
    ...
    if(!rec.date) return toast('Próxima fecha requerida');
...
function wirePersonal(){
  ...
  form?.addEventListener('submit',(ev)=>{
    ...
    const rec = {
      ...
      desc: $('#perDesc').value,
      amount: Number($('#perAmount').value||0)
    };
    if(!rec.date) return toast('Fecha requerida');
    state.personal.push(rec); save(); toast('Gasto personal guardado'); form.reset();
  });
...
/* ===================== Helpers para Reportes/Balances ===================== */
// Identifica si un gasto diario proviene de un recurrente (asiento automático) — evita doble conteo
...
/**
 * Suma gastos diarios separando recurrentes y no recurrentes, en un rango.
 * Devuelve { total, recurrent, nonRecurrent }
 */
...
// Suma pagos en un rango (todos los estados: Pendiente/Pagado)
...
// Suma gastos personales en un rango
...
function renderReports(){
  ...
  // Gastos (diarios split) + personales + pagos
  ...
  // Total gastos finales (sin duplicar): no-recurrentes + recurrentes + personales + pagos
...
function renderHome(){
  ...
  // Gastos del mes: diarios (split), personales y pagos
  const expMonthSplit = sumExpensesDailySplit(monthStart, today);
  const perMonth = sumPersonalRange(monthStart, today);
  const payMonth = sumPaymentsRange(monthStart, today);

  // Total gastos sin duplicar (los ordinarios ya entran en recurrentes de expensesDaily)
  ...
  $('#kpiExpensesMonth').textContent = fmt(totalExpenses);
  $('#kpiBalanceMonth').textContent  = fmt(balance);

  // Gráfico 12 meses: usamos ingresos vs gastos totales por mes
...
function wireCloudUI(){
  ...
  $('#btnSignIn')?.addEventListener('click', async ()=>{
    ...
    catch(e){ console.warn('Popup bloqueado, usando redirect', e); await signInWithRedirect(auth, provider); }
  });
  ...
  getRedirectResult(auth).catch(err=>{
    if(err && err.code !== 'auth/no-auth-event') console.warn('getRedirectResult:', err);
  });
  onAuthStateChanged(auth, (user)=>{ 
    cloud.user=user||null; uiCloud(); 
    if(user){ cloudSubscribe(); toast('Sesión iniciada'); } 
    else { cloud.unsub?.(); cloud.unsub=null; toast('Sesión cerrada'); }
  });
}
...
function init(){
  try{
    ...
    console.error(e);
    toast('Error inicializando la app. Revisa la consola.');
  }
}

/* ===================== DEBUG: expone API para consola ===================== */
const _cloudStatus = () => {
...
// usa `self` para que funcione igual que window
self.app = {
  cloudPush,          // subir local → Firestore
  cloudPull,          // bajar Firestore → local
  cloud,              // objeto (user, autosync, unsub)
  setAutosync: (v)=> setAutosync(!!v),
  state,              // estado local actual
  saveLocal: ()=> save({skipCloud:true}),
  cloudStatus: _cloudStatus
};
console.log("✅ API de depuración disponible como `app` (ej.: app.cloudPush(), app.cloudPull(), app.cloudStatus())");
