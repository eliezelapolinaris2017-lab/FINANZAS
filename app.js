// app.js (con Firebase integrado)
// ¡Pega tu firebaseConfig en el bloque indicado!
...
enableIndexedDbPersistence(db).catch(()=>{ /* opcional */ });
...
function wireOrdinary(){
  ...
  $('#addOrd')?.addEventListener('click', ()=>{
    ...
    $('#ordAmount')?.focus();
    document.getElementById('ordinaryForm')?.scrollIntoView({behavior:'smooth', block:'start'});
  });
}
...
async function cloudPull(replace=true){
  ...
  if(!snap.exists()){ toast('No hay datos en la nube aún'); return; }
  const remote = snap.data();
  const remoteUpdated = remote?._cloud?.updatedAt || 0;
  const localUpdated = state?._cloud?.updatedAt || 0;

  if(replace || remoteUpdated >= localUpdated){
    state = remote;
  }else{
...
let pushTimer;
function cloudPushDebounced(){ clearTimeout(pushTimer); pushTimer = setTimeout(cloudPush, 600); }
function cloudSubscribe(){
  ...
  cloud.unsub = onSnapshot(ref, (snap)=>{
    ...
    if((remote?._cloud?.updatedAt||0) > (state?._cloud?.updatedAt||0)){
      state = remote;
      save({skipCloud:true});
      toast('Actualizado desde la nube (tiempo real)');
    }
  });
}
