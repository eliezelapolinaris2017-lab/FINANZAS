// app.js — Añadido módulo de Facturas & Cotizaciones.
// Factura crea automáticamente un Ingreso vinculado (mismo formato).
// Cotizaciones no afectan totales. PDF B/N con TOTAL. Orden por fecha desc.
...
/* ===================== Estado / Utils ===================== */
const STORAGE_KEY = 'finanzas-state-v3';
...
function renderInvoices(){
  const tbody = $('#invoicesTable tbody'); if(!tbody) return; tbody.innerHTML=''; let sumMonth=0;
  const now=new Date(); const mStart=new Date(now.getFullYear(),now.getMonth(),1).toISOString().slice(0,10);
  ...
  // KPI mes
  sumMonth = state.invoices
    .filter(x=>inRange(x.date, mStart, today))
  ...
  $$('#invoicesTable [data-edit]').forEach(b=> b.onclick=()=> editInvoice(b.dataset.edit));
}
function deleteInvoice(id){
  const inv = state.invoices.find(x=>x.id===id); if(!inv){ toast('No encontrada'); return; }
  // borrar ingreso vinculado si existe
  if(inv.incomeId){
    state.incomesDaily = state.incomesDaily.filter(r=>r.id!==inv.incomeId);
  }
...
function editInvoice(id){
  ...
  r = ask(inv.client,'Cliente'); if(r.cancelled) return; inv.client = r.value||inv.client;
  r = ask(inv.method,'Método'); if(r.cancelled) return; inv.method = r.value||inv.method;
  r = askNumber(inv.amount,'Monto'); if(r.cancelled) return; inv.amount = r.value;
  r = ask(inv.note,'Nota'); if(r.cancelled) return; inv.note = r.value||inv.note;
  // sincroniza ingreso vinculado
  if(inv.incomeId){
    const inc = state.incomesDaily.find(x=>x.id===inv.incomeId);
    if(inc){
      inc.date = inv.date; inc.client = inv.client; inc.method = inv.method; inc.amount = inv.amount;
    }
  }
  save(); toast('Factura actualizada (sincronizada con Ingresos)');
}
function wireInvoices(){
  $('#invoiceForm')?.addEventListener('submit',(ev)=>{
    ev.preventDefault();
    const inv = {
      id:uid(), date: $('#invDate')?.value, number: $('#invNumber')?.value,
      client: $('#invClient')?.value, method: $('#invMethod')?.value,
      amount: Number($('#invAmount')?.value||0), note: $('#invNote')?.value
    };
    if(!inv.date || !inv.number) return toast('Fecha y número requeridos');

    // Crear ingreso vinculado
    const income = { id:uid(), date: inv.date, client: inv.client, method: inv.method, amount: inv.amount, invoiceNumber: inv.number };
  ...
  $('#addInvoiceToday')?.addEventListener('click', ()=>{ if($('#invDate')) $('#invDate').value = todayStr(); $('#invAmount')?.focus(); });
}
...
function renderQuotes(){
  ...
  state.quotes
    ...
    .forEach(q=>{
      ...
      tr.innerHTML = `
        ...
        <td>${q.note||''}</td>
        <td class="row-actions">
          <button class="btn-outline" data-edit="${q.id}">Editar</button>
          <button class="btn-outline" data-del="${q.id}">Eliminar</button>
        </td>`;
      tbody.appendChild(tr);
    });
  ...
  $$('#quotesTable [data-edit]').forEach(b=> b.onclick=()=> editQuote(b.dataset.edit));
}
function editQuote(id){
  ...
  r = ask(q.client,'Cliente'); if(r.cancelled) return; q.client = r.value||q.client;
  r = ask(q.method,'Método'); if(r.cancelled) return; q.method = r.value||q.method;
  r = askNumber(q.amount,'Monto'); if(r.cancelled) return; q.amount = r.value;
  r = ask(q.note,'Nota'); if(r.cancelled) return; q.note = r.value||q.note;
...
function wireQuotes(){
  $('#quoteForm')?.addEventListener('submit',(ev)=>{
    ev.preventDefault();
    const q = {
      id:uid(), date: $('#quoDate')?.value, number: $('#quoNumber')?.value,
      client: $('#quoClient')?.value, method: $('#quoMethod')?.value,
      amount: Number($('#quoAmount')?.value||0), note: $('#quoNote')?.value
    };
    if(!q.date || !q.number) return toast('Fecha y número requeridos');
    state.quotes.push(q); save(); toast('Cotización creada (no afecta totales)'); ev.target.reset();
  });
  $('#addQuoteToday')?.addEventListener('click', ()=>{ if($('#quoDate')) $('#quoDate').value = todayStr(); $('#quoAmount')?.focus(); });
}
...
/* ===================== PDF (jsPDF) — incluye Facturas/Cotizaciones ===================== */
...
async function generatePDF(view = "expenses") {
  ...
  const viewName = mapTitles[view] || view.toUpperCase();
  ...
  if (view === "expenses") {
    ...
    headers = ["Fecha","# Factura","Cliente","Método","Monto","Nota"];
    rows = state.invoices.map(f => [f.date, f.number, f.client, f.method, Number(f.amount||0).toFixed(2), f.note||""]);
    total = state.invoices.reduce((a,f)=>a + Number(f.amount||0), 0);
  } else if (view === "quotes") {
    headers = ["Fecha","# Cotización","Cliente","Método","Monto","Nota"];
    rows = state.quotes.map(q => [q.date, q.number, q.client, q.method, Number(q.amount||0).toFixed(2), q.note||""]);
    total = state.quotes.reduce((a,q)=>a + Number(q.amount||0), 0); // solo para PDF, no afecta balance
