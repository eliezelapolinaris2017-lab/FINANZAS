...
/* ===================== Estado / Utils ===================== */
const STORAGE_KEY = 'finanzas-state-v8';
...
/* ===================== Helpers de Ítems (Factura/Cotización) ===================== */
...
function readItemsFromTable(tbody){
  const items=[]; tbody.querySelectorAll('tr').forEach(tr=>{
...
function calcTotals(items){
  ...
  items.forEach(it=>{ const base=(it.qty||0)*(it.price||0); const tax=base*((it.tax||0)/100); subtotal+=base; taxTotal+=tax; });
...
function wireInvoicesCreate(){
  ...
  $('#invCalc')?.addEventListener('click', ()=>{
    ...
    $('#invSubtotal').textContent=fmt(t.subtotal); $('#invTaxTotal').textContent=fmt(t.taxTotal); $('#invGrandTotal').textContent=fmt(t.total);
  });
  $('#addInvoiceToday')?.addEventListener('click', ()=>{ if($('#invDate')) $('#invDate').value=todayStr(); });
  $('#invoiceForm')?.addEventListener('submit',(ev)=>{
    ev.preventDefault();
    const items=readItemsFromTable($('#invItemsTable tbody')); const t=calcTotals(items);
    const inv={ id:uid(), date:$('#invDate')?.value, dueDate:$('#invDueDate')?.value, number:$('#invNumber')?.value, method:$('#invMethod')?.value,
      client:{ name:$('#invClient')?.value, email:$('#invClientEmail')?.value, phone:$('#invClientPhone')?.value, address:$('#invClientAddress')?.value },
      items, subtotal:t.subtotal, taxTotal:t.taxTotal, total:t.total, note:$('#invNote')?.value, terms:$('#invTerms')?.value };
    ...
    ev.target.reset(); $('#invItemsTable tbody').innerHTML=''; $('#invSubtotal').textContent='—'; $('#invTaxTotal').textContent='—'; $('#invGrandTotal').textContent='—';
  });
}
...
function renderInvoicesHistory(q=''){
  ...
  state.invoices.slice().sort(byDateDesc).forEach(inv=>{
    ...
    const tr=document.createElement('tr');
    tr.innerHTML=`
      <td>${inv.date||''}</td><td>${inv.number||''}</td><td>${inv.client?.name||''}</td>
      <td>${fmt(inv.total||0)}</td><td>${inv.method||''}</td>
      <td class="row-actions">
...
function deleteInvoice(id){
  ...
  state.invoices = state.invoices.filter(x=>x.id!==id); save(); toast('Factura eliminada (y su ingreso)');
}
...
function renderQuotesKPI(){
  const now=new Date(); const mStart=new Date(now.getFullYear(),now.getMonth(),1).toISOString().slice(0,10); const today=now.toISOString().slice(0,10);
...
function wireQuotesCreate(){
  ...
  $('#quoCalc')?.addEventListener('click', ()=>{
    ...
    const t=calcTotals(readItemsFromTable(tb));
    $('#quoSubtotal').textContent=fmt(t.subtotal); $('#quoTaxTotal').textContent=fmt(t.taxTotal); $('#quoGrandTotal').textContent=fmt(t.total);
  });
  $('#addQuoteToday')?.addEventListener('click', ()=>{ if($('#quoDate')) $('#quoDate').value=todayStr(); });
  $('#quoteForm')?.addEventListener('submit',(ev)=>{
    ev.preventDefault();
    const items=readItemsFromTable($('#quoItemsTable tbody')); const t=calcTotals(items);
    const q={ id:uid(), date:$('#quoDate')?.value, validUntil:$('#quoValidUntil')?.value, number:$('#quoNumber')?.value, method:$('#quoMethod')?.value,
      client:{ name:$('#quoClient')?.value, email:$('#quoClientEmail')?.value, phone:$('#quoClientPhone')?.value, address:$('#quoClientAddress')?.value },
      items, subtotal:t.subtotal, taxTotal:t.taxTotal, total:t.total, note:$('#quoNote')?.value, terms:$('#quoTerms')?.value };
    if(!q.date || !q.number) return toast('Fecha y número requeridos');
    state.quotes.push(q); save(); toast('Cotización creada'); ev.target.reset();
    $('#quoItemsTable tbody').innerHTML=''; $('#quoSubtotal').textContent='—'; $('#quoTaxTotal').textContent='—'; $('#quoGrandTotal').textContent='—';
  });
}
function renderQuotesHistory(q=''){
  const tb=$('#quotesTable tbody'); if(!tb) return; tb.innerHTML=''; const s=(q||'').toLowerCase().trim();
  state.quotes.slice().sort(byDateDesc).forEach(qu=>{
    const hay = `${qu.number||''} ${qu.client?.name||''}`.toLowerCase(); if(s && !hay.includes(s)) return;
    const tr=document.createElement('tr');
    tr.innerHTML=`
      <td>${qu.date||''}</td><td>${qu.number||''}</td><td>${qu.client?.name||''}</td>
      <td>${fmt(qu.total||0)}</td><td>${qu.method||''}</td>
      <td class="row-actions">
...
function renderOrdinary(){
  ...
  state.ordinary.forEach(o=>{
    ...
    tr.innerHTML=`<td>${o.name}</td><td>${fmt(o.amount)}</td><td>${o.freq}</td><td>${o.next}</td>
      <td class="row-actions"><button class="btn-outline" data-edit="${o.id}">Editar</button><button class="btn-outline" data-del="${o.id}">Eliminar</button></td>`;
    tb.appendChild(tr);
  });
...
function renderBudgets(){
  ...
  state.budgets.forEach(b=>{
    ...
    <div class="row-actions"><button class="btn-outline" data-edit="${b.id}">Editar</button><button class="btn-outline" data-del="${b.id}">Eliminar</button></div>`;
...
function renderPersonal(){
  ...
  state.personal.slice().sort(byDateDesc).forEach(p=>{
    ...
    tr.innerHTML=`<td>${p.date||''}</td><td>${p.category||''}</td><td>${p.desc||''}</td><td>${fmt(p.amount)}</td>
      <td class="row-actions"><button class="btn-outline" data-edit="${p.id}">Editar</button><button class="btn-outline" data-del="${p.id}">Eliminar</button></td>`;
...
async function generatePDF(view="expenses", optionalId=null){
  ...
  const business=state.settings.businessName||"Mi Negocio"; const logo=state.settings.logoBase64||'assets/logo.png';
  ...
  function table(headers, rows, startY=42){ let y=startY; const colW=180/headers.length; doc.setFont("helvetica","bold"); doc.setFontSize(10); headers.forEach((h,i)=>doc.text(String(h),14+i*colW,y)); y+=6; doc.line(14,y,200,y); y+=6; doc.setFont("helvetica","normal"); rows.forEach(r=>{ r.forEach((c,i)=>doc.text(String(c??'').substring(0,32),14+i*colW,y)); y+=6; if(y>280){doc.addPage(); y=20;} }); return y; }
  function invoiceQuote(kind, rec){
    header(kind==='invoice'?'FACTURA':'COTIZACIÓN'); doc.setFont("helvetica","normal"); let y=42;
    doc.setFontSize(10); doc.setFont("helvetica","bold"); doc.text("Para:",14,y); doc.setFont("helvetica","normal");
    doc.text(String(rec.client?.name||''),14,y+6); if(rec.client?.email) doc.text(String(rec.client.email),14,y+12);
    if(rec.client?.phone) doc.text(String(rec.client.phone),14,y+18); if(rec.client?.address) doc.text(String(rec.client.address),14,y+24);
    let ry=42; const rightX=200;
    doc.setFont("helvetica","bold"); doc.text(kind==='invoice'?'Factura #':'Cotización #', rightX-70, ry); doc.setFont("helvetica","normal"); doc.text(String(rec.number||''), rightX-20, ry, {align:'right'}); ry+=6;
    doc.setFont("helvetica","bold"); doc.text("Fecha", rightX-70, ry); doc.setFont("helvetica","normal"); doc.text(String(rec.date||''), rightX-20, ry, {align:'right'}); ry+=6;
    if(kind==='invoice'){ doc.setFont("helvetica","bold"); doc.text("Vence", rightX-70, ry); doc.setFont("helvetica","normal"); doc.text(String(rec.dueDate||''), rightX-20, ry, {align:'right'}); ry+=6; }
    else { doc.setFont("helvetica","bold"); doc.text("Válida hasta", rightX-70, ry); doc.setFont("helvetica","normal"); doc.text(String(rec.validUntil||''), rightX-20, ry, {align:'right'}); ry+=6; }
    ...
    rec.items.forEach(it=>{ x=14; const base=(it.qty||0)*(it.price||0); const tax=base*((it.tax||0)/100); const amt=base+tax; const row=[it.desc||'', String(it.qty||0), Number(it.price||0).toFixed(2), String(it.tax||0), amt.toFixed(2)]; row.forEach((c,i)=>{ doc.text(String(c).substring(0,60),x,y); x+=colW[i]; }); y+=6; if(y>260){doc.addPage(); y=20;} });
    ...
    if(rec.note){ doc.setFont("helvetica","bold"); doc.text("Nota:",14,y); doc.setFont("helvetica","normal"); doc.text(String(rec.note).substring(0,240),14,y+6); y+=12; }
    if(rec.terms){ doc.setFont("helvetica","bold"); doc.text("Términos:",14,y); doc.setFont("helvetica","normal"); doc.text(String(rec.terms).substring(0,240),14,y+6); y+=12; }
  }
  if(view==='invoices' && optionalId){ const inv=state.invoices.find(x=>x.id===optionalId); if(!inv) return toast('Factura no encontrada'); invoiceQuote('invoice',inv); doc.save(`${business.replace(/\s+/g,'_')}_Factura_${inv.number||''}.pdf`); return; }
  if(view==='quotes' && optionalId){ const q=state.quotes.find(x=>x.id===optionalId); if(!q) return toast('Cotización no encontrada'); invoiceQuote('quote',q); doc.save(`${business.replace(/\s+/g,'_')}_Cotizacion_${q.number||''}.pdf`); return; }

  const titleMap={ payments:"PAGO DE NÓMINA", invoices:"FACTURAS", quotes:"COTIZACIONES", reconciliations:"CONCILIACIÓN BANCARIA" };
  const title = titleMap[view] || view.toUpperCase(); header(title);
  ...
  else if(view==="payments"){ headers=["Fecha","Empleado/Benef.","Categoría","Monto","Estado"]; rows=state.payments.map(p=>[p.date,p.to,p.category,Number(p.amount||0).toFixed(2),p.status]); total=state.payments.reduce((a,p)=>a+Number(p.amount||0),0); }
  ...
  doc.save(`${business.replace(/\s+/g,'_')}_${(title||view)}.pdf`);
}
...
let reconImportRows=[];  // [{date, desc, amount, ref?, match?}]
...
function wireAll(){
  ...
  wireReconciliationImport(); // Importador CSV
  initCatalogs(); applyTheme(); refreshAll(); showView('login');
}

/* ===================== API de consola (debug) ===================== */
