// app.js — App de Finanzas con Firebase (Auth + Firestore)
// Incluye: sincronización en la nube, edición por fila en todas las tablas,
// balance correcto (diarios + personales + pagos + recurrentes),
// y categoría extra en Gastos Diarios: “Equipos y Materiales”.
...
enableIndexedDbPersistence(db).catch(()=>{ /* ok si falla en incógnito */ });
...
// 🚨 Incluye “Equipos y Materiales” para Gastos Diarios
...
function initCatalogs(){
  ...
  upsertOptions($('#expMethod'), ["Efectivo","Tarjeta","Transferencia","ATH Móvil","Cheque"]);
  upsertOptions($('#incMethod'), INCOME_METHODS);
}
...
function askNumber(current, label){
  ...
  const n = parseFloat(a.value.replace(',','.'));
