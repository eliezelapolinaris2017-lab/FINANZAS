// app.js — App de Finanzas con Firebase (Auth + Firestore) y correcciones
// Requiere: <script type="module" src="app.js"></script> en index.html
...
// (Opcional extra seguridad) App Check reCAPTCHA v3
...
// Habilitar cache offline (si no se puede, no es fatal)
enableIndexedDbPersistence(db).catch(()=>{ /* ok */ });

// (Opcional) App Check — descomenta si creas la site key en Firebase
// const appCheck = initializeAppCheck(fbApp, {
//   provider: new ReCaptchaV3Provider("TU_SITE_KEY_DE_APP_CHECK"),
...
function applyTheme(){
  ...
  // Fallback de logo para evitar 404 si assets/logo.png no existe
...
function wireCloudUI(){
  ...
  // Iniciar sesión (popup → redirect fallback)
  ...
  // Resultado de redirect (no es error si no hay redirect)
