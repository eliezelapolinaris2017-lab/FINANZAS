...
/* ===================== Estado / Utils ===================== */
const STORAGE_KEY = 'finanzas-state-v4'; // bump
...
const DEFAULT_STATE = {
  ...
  expensesDaily: [],   // SOLOS CAMPOS: fecha, categoría, desc, method, amount, note
  incomesDaily: [],
  payments: [],
  ordinary: [],
  budgets: [],
  personal: [],
  invoices: [], // {id, number, date, dueDate, method, client{...}, items[], subtotal, taxTotal, total, note, terms, incomeId}
...
/* ===================== Gastos Diarios (SIN CAMBIOS DE CAMPOS) ===================== */
...
function renderInvoices(){
  ...
  sumMonth = state.invoices.filter(x=>inRange(x.date, mStart, today)).reduce((a,b)=>a+Number(b.total||0),0);
...
function editInvoiceBasic(id){
  ...
  save(); toast('Factura actualizada (básico)');
}
...
function renderQuotes(){
  ...
  state.quotes.slice().sort(byDateDesc).forEach(q=>{
    const tr=document.createElement('tr');
    tr.innerHTML = `
      <td>${q.date||''}</td>
      <td>${q.number||''}</td>
      ...
      <td>${q.method||''}</td>
      <td class="row-actions">
        <button class="btn-outline" data-pdf="${q.id}">PDF</button>
...
function editQuoteBasic(id){
  ...
  save(); toast('Cotización actualizada (básico)');
}
function wireQuotes(){
...
/* ===================== Ordinarios / Presupuestos / Personales (igual que antes) ===================== */
...
async function generatePDF(view = "expenses", optionalId=null) {
  ...
  function invoiceQuotePDF(kind, record){
    ...
    const colW = [90,20,30,20,20]; // suma aprox = 180
  ...
  // Tablas simples para exportación completa
  ...
  if (total !== null) {
    ...
    doc.setFont("helvetica","bold");
    doc.text("TOTAL", 14 + (180 - 40), y); // aprox al final
